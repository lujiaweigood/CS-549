...
//		routing.joinCheck();
	...
	public void join(String uri) throws Failed, Invalid {
		 ...
		 * pointing back to us. In the case  e
...
//		TableRep bindings = transferBindings(info.id);
		succ = findSuccessor(nodeAddress,info.id);
		state.clear();
		setSucc(succ);
		stabilize();
//		state.backupBindings(bindings);
	}
...
//		NodeInfo currentNode = getNodeInfo();
		EventSource eventSource = client.listenForBindings(keyNode, info.id, key);
		eventSource.register(listener);

//			EventListener moveEventListener = moveBindingListener(key, listener);
//			eventSource.register(listener, IDHTNode.NEW_BINDING_EVENT);
//			eventSource.register(moveEventListener, IDHTNode.MOVED_BINDING_EVENT);
		eventSource.open();
...
//	private EventListener moveBindingListener(String key, EventListener listener){
//		return new EventListener() {
//			@Override
//			public void onEvent(InboundEvent inboundEvent) {
//				if (inboundEvent.getName().equals(IDHTNode.MOVED_BINDING_EVENT)) {
//					try {
////					System.out.println((String.format("** Binding event (%s) : %s -> %s", inboundEvent.getId())));
//						msgln(String.format("** Binding event (%s): %s -> %s", inboundEvent.getName(), key, inboundEvent.readData(String.class)));
...
//	}
	...
	public void listenOff(String key) throws DHTBase.Failed {
		...
		NodeInfo keyNode = findSuccessor(id);
//		NodeInfo currentNode = getNodeInfo();
		client.listenOff(keyNode, info.id, key);
