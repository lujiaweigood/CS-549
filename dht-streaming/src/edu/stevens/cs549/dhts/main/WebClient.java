...
public class WebClient {
	...
	public NodeInfo getSucc(NodeInfo node) throws DHTBase.Failed {
		...
		if(response == null || response.getStatus() >=300) {
			...
			NodeInfo succ = response.readEntity(node.getClass());
			return succ;
		}
	}
	...
	public NodeInfo closestPrecedingFinger(NodeInfo node,int id) throws DHTBase.Failed  {
		...
		if(response == null || response.getStatus() >=300) {
			...
			NodeInfo closestPrecedingFinger = response.readEntity(node.getClass());
...
//					.header(Time.TIME_STAMP, Time.advanceTime())
	...
	public String[] get(NodeInfo node, String skey) throws DHTBase.Failed {
		...
		URI get = ub.queryParam("key", skey).build();
		info("client get("+get+")");
	...
	public void add(NodeInfo node, String skey, String v) throws DHTBase.Failed {
		UriBuilder ub = UriBuilder.fromUri(node.addr);
		URI add = ub.queryParam("key", skey).queryParam("val", v).build();
		TableRep tablerep = new TableRep(null, null, 1);
		tablerep.entry[0] = new TableRow(skey, new String[]{v});
		info("client add("+add+")");
		Response response = putRequest(add, tablerep);
//		Response response = putRequest(add);
		if(response == null || response.getStatus() >=300) {
	...
	public void delete(NodeInfo node, String skey, String v) throws DHTBase.Failed {
		UriBuilder ub = UriBuilder.fromUri(node.addr);
		URI delete = ub.queryParam("key", skey).queryParam("val", v).build();
		info("client delete("+delete+")");
...
//		URI url = null;
//		//new EventSource();
//		WebTarget target = listenClient.target(url);
