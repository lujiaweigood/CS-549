...
public class Client {
	...
	public Client(String[] args) {
		try {
			...
			IServerFactory root = (IServerFactory)registry.lookup(serverName);
			IServer server = root.createServer();
...
//			InetAddress myAddr = InetAddress.getByName(clientIp);
			log.info("Client binding to server socket at address ");
			InetAddress myAddr = InetAddress.getByName(clientIp);
		...
		private void makePassive(InetSocketAddress serverPort) {
			serverSocket = InetSocketAddress.createUnresolved(serverAddr, serverPort.getPort());
			mode = Mode.PASSIVE;
		}
				...
				try {
					if (mode == Mode.PASSIVE) {
						...
						try {
							 ...
							 */
							InputStream is = new BufferedInputStream(socket.getInputStream()); //Read from socket
							byte[] buf = new byte[4096];
							int nbytes;
							while((nbytes = is.read(buf))>0) {
								out.write(buf,0,nbytes);
			...
			if (inputs.length == 2) {
				try {
					if (mode == Mode.PASSIVE) {
						...
						Socket socket= new Socket(serverAddress, serverSocket.getPort());
						svr.put(inputs[1]);
						InputStream fi = new BufferedInputStream(new FileInputStream(inputs[1]));
						OutputStream os = new BufferedOutputStream(socket.getOutputStream());
						byte[] buffer = new byte[4096];
						int nbytes;
						while((nbytes = fi.read(buffer))>0) {
							os.write(buffer,0,nbytes);
						}
						fi.close();
						os.close();
						 ...
						 */
						InputStream fi = new BufferedInputStream(new FileInputStream(inputs[1]));
						new Thread(new PutThread(dataChan,fi)).start();
						svr.put(inputs[1]);
					} else {
						msgln("GET: No mode set--use port or pasv command.");
					}
